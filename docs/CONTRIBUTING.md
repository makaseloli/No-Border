...
Subprojects are the top-level directories corresponding to each platform/version (such as `26.1.2-fabric` or `1.20.1-forge`).

| Changes affect...                            | Scope                                                                  |
|----------------------------------------------|------------------------------------------------------------------------|
| A single subproject                          | Use that subproject's directory name (e.g. `fix(26.1.2-fabric): ...`)  |
...
```
feat(26.1.2-fabric): add config screen
fix(1.20.1-forge): resolve crash on startup
build: upgrade Gradle to 8.14
chore(1.20.1-forge,1.21.1-neo): bump loader versions
fix(1.20.1): resolve crash affecting all 1.20.1 loaders
feat(fabric): add Fabric loader support across all versions
```
