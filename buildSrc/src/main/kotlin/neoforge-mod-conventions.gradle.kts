...
tasks.jar {
    ...
    from(project(commonProject).layout.projectDirectory.file("src/main/resources/$modId.mixins.json")) {
        filter(org.gradle.api.Transformer<String?, String> { line ->
            val text = line ?: ""
            if (text.trim().startsWith("\"refmap\"")) "" else text
        })
    }
}
